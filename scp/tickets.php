...
if($_POST && !$errors):
    ...
    if($ticket && $ticket->getId()) {
		...
		case 'mergeticket': //merge-tickets 
		    $mTicket = $ticket;
		    $mTicketRole = $role;
			...
			if($_REQUEST['keepTicket'] || $_REQUEST['manKeepTicket']) {
				if($_REQUEST['keepTicket'] && !($kTicket=Ticket::lookup($_REQUEST['keepTicket'])))
					$errors['err']=sprintf(__('%s: Unknown or invalid ID.'), __('Selected Ticket'));
				elseif($_REQUEST['manKeepTicket'] && !($kTicket=Ticket::lookup(['number' => $_REQUEST['manKeepTicket']])))
					$errors['err']=sprintf(__('%s: Unknown or invalid number.'), __('Selected Ticket'));
				elseif(!$kTicket->checkStaffPerm($thisstaff)) {
					$errors['err']=__('Selected Ticket').': '.__('Access denied. Contact admin if you believe this is in error');
					$kTicket=null; //Clear ticket obj.
				}
			}
			else {
				$errors['err']=__('No valid ticket to merge selected');
				break;
			}
			if(!$errors) { // check permissions
				$kTicketRole = $thisstaff->getRole($kTicket->getDeptId());
				$kTicketLock = $kTicket->getLock(); //Ticket lock if any
				
            	// can staff edit both tickets?
				if(!$mTicketRole->hasPerm(TicketModel::PERM_EDIT) || !$kTicketRole->hasPerm(TicketModel::PERM_EDIT))
            	    $errors['err']=__('Permission Denied. You are not allowed to edit one of these tickets');
				elseif(!$mTicket->isOpen() || !$kTicket->isOpen())
					$errors['err']=__('Permission Denied. At least one of these tickets is closed.');
				else { // if there any locks?
        			if($mTicket->isLocked() && ($mLock=$mTicket->getLock()) && !$mLock->isExpired()) {
            			if ($mLock->getStaffId() != $thisstaff->getId()) //Ticket is locked by someone else.??
							$errors['err'] = sprintf(__('Permission denied for ticket #%s'), $mTicket->getNumber()).' '
											.sprintf(__('Currently locked by %s'), $mLock->getStaff()->getName());
						else //Ticket already locked by staff...try renewing it.
							$mLock->renew(); //New clock baby!
        			}
        			elseif($kTicket->isLocked() && ($kLock=$mTicket->getLock()) && !$kLock->isExpired()) {
            			if ($kLock->getStaffId() != $thisstaff->getId()) //Ticket is locked by someone else.??
							$errors['err'] = sprintf(__('Permission denied for ticket #%s'), $kTicket->getNumber()).' '
											.sprintf(__('Currently locked by %s'), $kLock->getStaff()->getName());
						else //Ticket already locked by staff...try renewing it.
							$kLock->renew(); //New clock baby!
        			}
				}
			}
			if(!$errors && ($mergeResult=$mTicket->mergeTicket($kTicket))) {
            	$msg=__('Tickets merged sucessfully');
            	$redirect = 'tickets.php?id='.$kTicket->getId();
            	$_REQUEST['a'] = null; //Clear edit action - going back to view.
            } elseif(!$errors['err']) {
                $errors['err']=__('Unable to merge tickets. Any Error occured.');
