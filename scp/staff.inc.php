...
/*
$ds['deptID']    - selected Dept-ID - 0 = all
$ds['depts']   - Array with available Depts (ID=>name)
$ds['deptIDs'] - Array of available DeptIDs */

if(isset($_REQUEST['dsId']) && intval($_REQUEST['dsId']) == $_REQUEST['dsId']) 
	$ds['deptId'] = $_REQUEST['dsId'];
elseif(isset($_SESSION['dsId']) && intval($_SESSION['dsId']) == $_SESSION['dsId']) 
	$ds['deptId'] = $_SESSION['dsId'];
else
	$ds['deptId'] = 0;
...
if($ds['deptId'] == 0)
	...
	$_SESSION['dsId'] = $ds['deptId'];
	
// Bei status = search -> dsId = 0
if(isset($_REQUEST['advanced']) || (isset($_REQUEST['status']) && $_REQUEST['status'] == 'search'))
	$ds['deptId'] = 0;

$ds['depts'][0] = __('All');
$ds['deptIDs'] = array();

if(!$ost->isUpgradePending() &&$depts=Dept::getDepartments()) {
	$staffdepts=$thisstaff->getDepts();// IDs of Staff-Depts
	// be sure, only Depts are displayed, where staff has access...
	$hiddenDepts = array();// hide this Depts
	foreach($depts as $id => $name) {
		if(in_array($id, $staffdepts) && !in_array($id, $hiddenDepts)) { // diese Abteilungen ausblenden
			$ds['depts'][$id] = $name;
		}
	}

	ksort($ds['depts']);
	foreach($ds['depts'] as $id => $name) {
		$ds['deptIDs'][] = $id;
	}
}
