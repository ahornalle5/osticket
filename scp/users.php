...
// ----------------- TAPI-Funktion - Anfang -----------------------------------------------
if($_REQUEST['tapi']){
	#echo 'Anrufer-Tel: '.$_REQUEST['callerTel'].'<br>';
	#echo 'angerufene Nr.: '.$_REQUEST['calledTel'].'<br>';
	#echo 'Anrufer-Name: '.$_REQUEST['callerName'].'<br>';
	if($_REQUEST['tapi'] == 'urldecoded') { //codiert
		trim($_REQUEST['callerTel']);
		trim($_REQUEST['calledTel']);
		trim($_REQUEST['callerName']);
	}
	else {	// unbekannt und nicht codiert -> Leerzeichen durch + ersetzen
			// Superglobals werden decodiert, dabei wird '+' zu Leerzeichen...
		$_REQUEST['callerTel'] = preg_replace("/[ ]/", "+", $_REQUEST['callerTel']);
		$_REQUEST['calledTel'] = preg_replace("/[ ]/", "+", $_REQUEST['calledTel']);
		$_REQUEST['callerName'] = preg_replace("/[ ]/", "+", $_REQUEST['callerName']);
	}
	#echo 'Anrufer-Tel: '.$_REQUEST['callerTel'].'<br>';
	#echo 'angerufene Nr.: '.$_REQUEST['calledTel'].'<br>';
	#echo 'Anrufer-Name: '.$_REQUEST['callerName'].'<br>';
	if(substr($_REQUEST['callerTel'], 0, 1) == '+') {
		$callerTelRaw = preg_replace("/[^0-9]/", "", $_REQUEST['callerTel']);
		$callerTelFull = "+".$callerTelRaw;
		$callerTel = '0'.substr($callerTelFull, 3);
		$callerTelSearch = substr($callerTelFull, 3);
	}
	elseif(substr($_REQUEST['callerTel'], 0, 2) == '00') {
		$callerTelRaw = $callerTelFull = preg_replace("/[^0-9]/", "", $_REQUEST['callerTel']);
		$callerTel = '0'.substr($callerTelFull, 4);
		$callerTelSearch = substr($callerTelFull, 4);
	}
	else {
		$callerTelRaw = $callerTelFull = $callerTel = preg_replace("/[^0-9]/", "", $_REQUEST['callerTel']);
		$callerTelSearch = substr($callerTelFull, 1);
	}
	if(substr($_REQUEST['calledTel'], 0, 1) == '+') {
		$calledTelRaw = preg_replace("/[^0-9]/", "", $_REQUEST['calledTel']);
		$calledTelFull = "+".$calledTelRaw;
		$calledTel = '0'.substr($calledTelFull, 3);
	}
	elseif(substr($_REQUEST['calledTel'], 0, 2) == '00') {
		$calledTelRaw = $calledTelFull = preg_replace("/[^0-9]/", "", $_REQUEST['calledTel']);
		$calledTel = '0'.substr($calledTelFull, 4);
	}
	else {
		$calledTelRaw = $calledTelFull = $calledTel = preg_replace("/[^0-9]/", "", $_REQUEST['callerTel']);
	}
	if(isset($_REQUEST['callerName']))
		$callerName = $_REQUEST['callerName'];
	
	$_REQUEST['a'] = 'search';
	$_REQUEST['query'] = $callerTelSearch;
	#echo 'Anrufer-Tel raw: '.$callerTelRaw.'<br>';
	#echo 'Anrufer-Tel full: '.$callerTelFull.'<br>';
	#echo 'Anrufer-Tel: '.$callerTel.'<br>';
	#echo 'angerufene Nr. raw: '.$calledTelRaw.'<br>';
	#echo 'angerufene Nr. full: '.$calledTelFull.'<br>';
	#echo 'angerufene Nr.: '.$calledTel.'<br>';
	#echo 'Anrufer-Name: '.$callerName.'<br>';
	#exit;
}
// ----------------- TAPI-Funktion - Ende -----------------------------------------------
...
if(!isset($_REQUEST['tapi']))// TAPI-Funktion
