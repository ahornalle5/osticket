...
if($_REQUEST['tapi']) {
	if($users->exists(true) && ($users->count() == 1)) {
		// Telefonnummer eindeutig - Kundenfenster öffnen
		$url = "./users.php?id=".$U[0]['id'];
		header("Location: $url");
		exit();
	}
	...
	elseif($users->count() == 0) {
		...
		$url = "tickets.php?a=open&tel=".urlencode($callerTelFull)."&search=".urlencode($callerTelSearch);
		if(isset($callerName))
			$url .= '&name='.urlencode($callerName);
		header("Location: $url");
		exit();
	}
}
