...
if (!$thisstaff->hasPerm(SearchBackend::PERM_EVERYTHING)) {
    ...
    if ($thisstaff->showAssignedOnly()) 
    	$tickets->filter(Q::any($visibility))->filter(Q::all(new Q(array('dept_id__in' => $depts, 'staff_id' => $thisstaff->getId()))));
	else
 ...
 <table class="list" border="0" cellspacing="1" cellpadding="2" width="940">
    <thead>
        <tr>
            ...
            <th width="2%">&nbsp;</th><!-- Header Fälligkeitsampel -->
    ...
    foreach($tickets as $T) {
        ...
        <tr id="<?php echo $T['ticket_id']; ?>">
			...
			$dueDate = strtotime($T['duedate']);
			$displayDueDate = Format::datetime($T['duedate']);
			if(!$dueDate && intval($T['sla']) > 0) {
				$dueDate = strtotime($T['est_duedate']);
				$displayDueDate = Format::datetime($T['est_duedate']);
			}
			if($dueDate && !$T['isoverdue'] && strcasecmp($T['status__state'],'closed') != 0) {
				if($dueDate < strtotime("now")) { // unter 1 Std -> rot blinkend
					echo '<img title="Das Ticket ist bereits überfällig ('.$displayDueDate.')" class="Icon overdueTicket">';
				}
			...
			elseif($dueDate && $T['isoverdue'] && strcasecmp($T['status__state'],'closed') != 0) {
				echo '<img src="./images/dot_red_blink15x15.gif" alt="" title="Das Ticket ist bereits überfällig ('.$displayDueDate.')">';
			}
