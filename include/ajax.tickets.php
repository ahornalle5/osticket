...
class TicketsAjaxAPI extends AjaxController {
    ...
    function lookup() {
        ...
        if ($depts=$thisstaff->getDepts()) {//if (!$thisstaff->showAssignedOnly() && ($depts=$thisstaff->getDepts())) {
    	...
    	if ($thisstaff->showAssignedOnly() && ($depts=$thisstaff->getDepts())) {
	        $hits = TicketModel::objects()
	            ->filter($visibility)
	            ->filter(Q::all(array('dept_id__in' => $depts, 'staff_id' => $thisstaff->getId())))
    ...
    function updateUser($tid) {
        ...
        if(!$thisstaff
            ...
            Http::response(404, __('No such ticket/user'));
    ...
    function manageForms($ticket_id) {
        ...
        if (!$thisstaff)
            Http::response(403, __("Login required"));
    ...
    function updateForms($ticket_id) {
        ...
        elseif (!isset($_POST['forms']))
            Http::response(422, __("Send updated forms list"));
        ...
        Http::response(201, __('Successfully managed'));
    }
    ...
    function cannedResponse($tid, $cid, $format='text') {
        ...
        if (!($ticket = Ticket::lookup($tid))
                || !$ticket->checkStaffPerm($thisstaff))
            Http::response(404, __('Unknown ticket ID'));


        if ($cid && !is_numeric($cid)) {
            if (!($response=$ticket->getThread()->getVar($cid)))
                Http::response(422, __('Unknown ticket variable'));
        ...
        if (!$cid || !($canned=Canned::lookup($cid)) || !$canned->isEnabled())
            Http::response(404, __('No such premade reply'));
    ...
    function massProcess($action, $w=null)  {
        ...
        if ($_POST && $i) {
            ...
            Http::response(201, __('processed'));
    ...
    function setTicketStatus($tid) {
        ...
        if (!$errors && $ticket->setStatus($status, $_REQUEST['comments'], $errors)) {
            ...
            Http::response(201, __('Successfully processed'));
        } elseif (!$errors['err']) {
    ...
    function triggerThreadAction($ticket_id, $thread_id, $action) {
        ...
        if (!$thread)
            Http::response(404, __('No such ticket thread entry'));
        ...
        if (!$valid)
            Http::response(400, __('Not a valid action for this thread'));
