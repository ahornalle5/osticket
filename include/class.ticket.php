...
implements RestrictedAccess, Threadable {
    ...
    function checkStaffPerm($staff, $perm=null) {
        ...
        if (($staff->showAssignedOnly()
            ...
            //&& $this->isOpen() // Anpassung MAs mit limited Access zeige Geschlossene
   ...
   function onOverdue($whine=true, $comments="") {
    ...
    function pdfExport($psize='A4', $notes=false) {
        global $thisstaff;
        ...
        $name = __('Ticket').'-'.$this->getNumber().'.pdf';
   ...
   /*============== Static functions. Use Ticket::function(params); =============nolint*/
    ...
    function getStaffStats($staff) {
        ...
        if ($depts = $staff->getDepts())//if (!$staff->showAssignedOnly() && ($depts = $staff->getDepts()))
	    ...
	    if ($staff->showAssignedOnly() && ($depts=$staff->getDepts())) {
	        $blocks = Ticket::objects()
    	        ...
    	        ->filter(Q::all(new Q(array('dept_id__in' => $depts, 'staff_id' => $staff->getId()))))				
		...
		if($ds['deptId'] > 0)
    		$blocks->filter(array('dept_id' => $ds['deptId']));
    ...
    static function checkOverwaiting() {
        $sql='SELECT ticket_id FROM '.TICKET_TABLE.' T1 '
            ...
            .' AND '
            .' ( '
            .'     (ifnull(T1.duedate, T1.est_duedate) is null  '
