...
implements TemplateVariable {
    ...
    */
    ...
    static function create($vars=false) {
		...
		// Timesheet: Zeit speichern
		$tsVars = array();
		$tsVars['a'] = $vars['a'];
		$tsVars['response'] = $vars['response'];
		$tsVars['thread_id'] = $entry->thread_id; //ID des Verlaufs
		$tsVars['thread_entry_id'] = $entry->id; //ID des Verlaufeintrags
		$tsVars['thread_entry_type'] = $entry->type;
		$tsVars['object_id'] = $tsVars['id'];
		$tsVars['staff_id'] = $entry->staff_id;
		$tsVars['processingTime'] = $vars['ptHiddenFieldValue'];
		$tsVars['processingTime_type_id'] = $vars['ptInputFieldType'];
		$tsVars['created'] = $entry->created;
		require_once(INCLUDE_DIR.'class.timesheet.php'); // Timesheet include
		if(TS::saveThreadTime($tsVars)) {
			$vars['ptHiddenFieldValue'] = 0;// Zeit löschen, damit Sie nur einmal gespeichert wird
		}
