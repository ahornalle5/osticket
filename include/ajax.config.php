...
class ConfigAjaxAPI extends AjaxController {
    ...
    function templateLinks() {
        $links = $this->json_encode(array(
            array('name'=>__('Select').' ...', 'url'=> false),
            array('name'=>__('Agent Ticket Link'), 'url'=> '%{ticket.staff_link}'),
            array('name'=>__('Agent Login Page'), 'url'=> '%{url}/scp'),
            array('name'=>__('End-User Ticket Link'), 'url'=> '%{recipient.ticket_link}'),
            array('name'=>__('End-User Login Page'), 'url'=> '%{url}/login.php'),
        ));
